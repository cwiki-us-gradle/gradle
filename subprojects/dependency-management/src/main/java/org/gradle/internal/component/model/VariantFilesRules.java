...
public class VariantFilesRules {
    ...
    public <T extends ModuleComponentArtifactMetadata> ImmutableList<T> execute(VariantResolveMetadata variant, ImmutableList<T> artifacts, ModuleComponentIdentifier componentIdentifier) {
        DefaultMutableVariantFilesMetadata files = new DefaultMutableVariantFilesMetadata();
        for (VariantMetadataRules.VariantAction<? super MutableVariantFilesMetadata> action : actions) {
            action.maybeExecute(variant, files);
        }
        if (files.getFiles().isEmpty()) {
            return artifacts;
        }
        ImmutableList.Builder<T> newArtifacts = new ImmutableList.Builder<>();
        newArtifacts.addAll(artifacts);
        for (VariantFileMetadata file : files.getFiles()) {
            newArtifacts.add(Cast.<T>uncheckedNonnullCast(new UrlBackedArtifactMetadata(componentIdentifier, file.getName(), file.getUrl())));
        }
